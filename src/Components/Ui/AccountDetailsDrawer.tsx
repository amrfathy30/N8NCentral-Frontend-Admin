...
export default function AccountDetailsDrawer({ isOpen, onClose, account }: AccountDetailsDrawerProps) {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex overflow-hidden">
            {/* Backdrop */}
            <div
                className={`fixed inset-0 bg-black/30 backdrop-blur-[2px] transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}
                onClick={onClose}
            />

            {/* Sidebar Content */}
            <div
                className={`fixed top-0 bottom-0 w-full max-w-[550px] bg-[#F8F9FA] shadow-2xl transition-transform duration-300 ease-in-out flex flex-col ${dir === 'rtl'
            ...
            >
                ...
                <div className="flex items-center justify-between p-4">
                    <h2 className="text-[20px] font-bold text-greenDark">{t("AccountDetails.Title")}</h2>
                ...
                {/* Footer Actions */}
                <div className="p-6 border-t border-gray-50 bg-[#F9FAFB] space-y-3">
